...
export function printOrderInvoice(order: Order): void {
  ...
  <h1>MESCO</h1><div class="m">Invoice for order ${order.id.slice(-6).toUpperCase()} · ${new Date(order.createdAt).toLocaleDateString()}</div>
  <div><b>Customer:</b> ${order.customerName}</div>
  <table><thead><tr><th>Item</th><th>Series</th><th>Qty (box)</th><th>Price</th><th>Line total</th></tr></thead><tbody>
  ${order.items.map(i => `<tr><td>${i.name}</td><td>${SERIES_LABEL[i.series as Series] || i.series}</td><td>${i.qty}</td><td>${fmt(i.price)}</td><td>${fmt((i.price || 0) * i.qty)}</td></tr>`).join('')}
  </tbody></table>
...
export function printPackingSlip(order: Order): void {
  ...
  <table><thead><tr><th>#</th><th>Item Name</th><th>Item Code</th><th>Series</th><th>Color</th><th>Quantity (Boxes)</th></tr></thead><tbody>
  ${order.items.map((i, idx) => `<tr><td>${idx + 1}</td><td>${i.name}</td><td>${i.code}</td><td>${SERIES_LABEL[i.series as Series] || i.series}</td><td>${i.color || 'Standard'}</td><td><b>${i.qty} box(es)</b></td></tr>`).join('')}
  </tbody></table>
  </body></html>`);
  w.document.close();
  w.print();
