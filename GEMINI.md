...
9. **User Account Creation & Role Management:** Only `Admin` can create accounts, edit roles (`admin`, `manager`, `store`, `customer`), or delete users. There is no public registration/self-signup. Login requires username & password only; role tabs are omitted and backend auto-routes users upon authentication.
